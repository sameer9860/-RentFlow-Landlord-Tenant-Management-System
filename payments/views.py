...
class ExportReportPDFView(LandlordRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        # Pie Chart
        if revenue > 0 or total_expense > 0 or due_summary > 0:
            d = Drawing(400, 200)
            pc = Pie()
            pc.x = 100
            pc.y = 20
            pc.width = 160
            pc.height = 160
            pc.data = [revenue, total_expense, due_summary]
            pc.labels = ['Revenue', 'Expenses', 'Due']
            pc.slices[0].fillColor = colors.HexColor('#28a745')
            pc.slices[1].fillColor = colors.HexColor('#dc3545')
            pc.slices[2].fillColor = colors.HexColor('#ffc107')
            d.add(pc)
            
            elements.append(Paragraph("Financial Distribution", styles['Heading2']))
            elements.append(d)
            elements.append(Spacer(1, 48))
